...
const Sidebar = () => {
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              to={item.href}
              className={`
            group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px]
  ...
  return (
    <>
      ...
      {/* Sidebar - Desktop */}
      <aside className="hidden lg:flex flex-col h-full w-65 bg-linear-to-b from-slate-900 to-slate-950 text-white shrink-0 border-r border-white/4">
        {sidebarContent}
      </aside>

      {/* Sidebar - Mobile */}
      <aside
        className={`lg:hidden fixed inset-y-0 w-72 bg-linear-to-b
            from-slate-900 via-slate-900 to-slate-950 text-white z-50 flex flex-col transform duration-300
            {
