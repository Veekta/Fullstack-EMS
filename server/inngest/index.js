...
const autoCheckOut = inngest.createFunction(
  { id: "auto-check-out", triggers: [{ event: "employee/checked-out" }] },
  async ({ event, step }) => {
      ...
      await sendEmail({
        ...
        body: `
                <div style="max-width: 600px;">
                    ...
                    <p style="font-size: 16px;">You have a check-out in ${employee.department} today:</p>
  ...
  async ({ step }) => {
    ...
    const onLeaveIds = await step.run("get-on-leave-ids", async () => {
      const leaves = await LeaveApplication.find({
        ...
        endUTC: { $gte: new Date(today.startUTC) },
      }).lean();
    ...
    const absentEmployees = activeEmpoyees.filter(
      (emo) =>
        !onLeaveIds.includes(employeesRouter._id) &&
        !checkedInIds.includes(employeesRouter._id),
    );
